...
const ExecutiveLayout = () => {
    ...
    const handleNotificationClick = async (n) => {
        ...
        if (n.type === 'lead' || n.type === 'web_lead') navigate('/executive/leads');
        if (n.type === 'appointment') navigate('/executive/appointments');
            ...
            <nav className={`admin-sidebar bg-white shadow-sm d-flex flex-column ${isMobileSidebarOpen ? 'mobile-open' : ''}`}>
                ...
                <div className="p-3 border-top mt-auto bg-light-subtle">
                    <div className="d-flex align-items-center mb-3 px-2">
                        <div className="bg-warning-subtle text-warning rounded-circle d-flex align-items-center justify-content-center fw-bold me-2" style={{ width: '35px', height: '35px', fontSize: '0.8rem' }}>
                            {user?.name ? user.name.split(' ').map(n => n[0]).join('').toUpperCase() : 'E'}
                        </div>
                        <div className="flex-grow-1 overflow-hidden">
                            <h6 className="mb-0 text-truncate" style={{ fontSize: '0.85rem', fontWeight: 'bold' }}>{user?.name || 'Executive'}</h6>
                        </div>
                    </div>
