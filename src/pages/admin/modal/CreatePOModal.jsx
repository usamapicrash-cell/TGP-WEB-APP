...
const CreatePOModal = ({ show, onClose, lead, leadId, onSuccess, editingPO }) => {
    ...
    }, [sendEmail, supplierId, items, suppliers, notes]);
    ...
    return (
        <div className="modal show d-block" style={{ backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 1050 }}>
            <div className="modal-dialog modal-lg modal-dialog-centered mx-2 mx-sm-auto">
                <div className="modal-content border-0 shadow-lg" style={{ borderRadius: '15px', overflow: 'hidden' }}>
                    
                    {/* Header */}
                    <div className="modal-header bg-white border-bottom p-3 p-md-4 align-items-center">
                        <div>
                    ...
                    {/* Body */}
                    <div className="modal-body p-3 p-md-4" style={{ maxHeight: '75vh', overflowY: 'auto' }}>
                                ...
                                <div className="d-flex gap-2 flex-wrap">
                                    {existingDrawings.map((path, i) => (
                                        <div key={`ex-d-${i}`} className="position-relative border bg-white rounded p-1">
                                            ...
                                            <span 
                                                onClick={() => handleDirectDelete(path, 'drawing')} 
                                    ...
                                    {existingAttachments.map((path, i) => {
                                        ...
                                        return isImage ? (
                                            <div key={`ex-f-${i}`} className="position-relative border bg-white rounded p-1 shadow-sm">
                                                ...
                                                <span 
                                                    onClick={() => handleDirectDelete(path, 'attachment')} 
                                                    className="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger" 
                                                    style={{ cursor: 'pointer', fontSize: '12px' }}
                                                >×</span>
                                            </div>
                        ...
                        <div className="border rounded-3 overflow-hidden mb-4 shadow-sm">
                                        ...
                                        {items.map((item, index) => (
                                            <tr key={index} className="border-top">
                                                ...
                                                <td>
                                                    <input 
                                                        type="number" 
                                                        className="form-control form-control-sm border-0 bg-light text-center fw-bold shadow-none px-1" 
                                                        value={item.quantity} 
                                                ...
                                                <td>
                                                    <button 
                                                        type="button" 
                                                        className="btn btn-link btn-sm text-danger shadow-none p-0" 
                            ...
                            <div className="p-3 border-top bg-light d-flex flex-column flex-sm-row justify-content-between align-items-sm-center gap-2">
                                <button 
                                    type="button" 
                                    className="btn btn-link btn-sm fw-bold text-decoration-none shadow-none p-0 text-start" 
                        ...
                        <div className="border rounded-3 overflow-hidden bg-white shadow-sm mb-2">
                            <div className="d-flex flex-wrap align-items-center justify-content-between p-2 border-bottom bg-light gap-2">
                                        ...
                                        {['#2b3a67', '#198754', '#dc3545', '#000000'].map(c => (
                                            <div 
                                                ...
                                                style={{ 
                                                    ...
                                                    borderRadius: '50%', 
                                                    cursor: 'pointer', 
                                                    border: strokeColor === c ? '2px solid #fff' : 'none', 
                                                    outline: strokeColor === c ? '1px solid #2b3a67' : 'none' 
                                                }} 
                                            />
                                        ))}
                                    </div>
                                ...
                                <div className="d-flex gap-1 ms-auto">
                                    ...
                                    <button type="button" onClick={handleSaveSketch} className="btn btn-primary btn-sm ms-1 fw-bold" style={{ fontSize: '0.65rem' }}>ADD SKETCH</button>
                                </div>
                            </div>
                        ...
                        <div className="mb-3 mt-4 p-3 border rounded-3 bg-light">
                            <div className="form-check form-switch d-flex align-items-center gap-2 p-0">
                                <input 
                                    className="form-check-input shadow-none ms-0" 
                                    type="checkbox" 
                                    id="sendEmailCheck"
                                    checked={sendEmail}
                                ...
                                />
                                <label className="form-check-label small fw-bold" htmlFor="sendEmailCheck" style={{ cursor: 'pointer' }}>
                            ...
                            {sendEmail && (
                                <div className="mt-3 animate__animated animate__fadeIn">
                                    <div className="bg-white border rounded shadow-sm overflow-hidden">
                                        ...
                                        ></textarea>
                                        <div className="p-2 border-top bg-light text-muted smaller">
                    ...
                    {/* Footer */}
                    {/* Footer */}
                    <div className="modal-footer border-0 p-3 p-md-4 pt-0 d-flex justify-content-end gap-2">
                        <button 
                            type="button" 
                            className="btn btn-light px-4 py-2 fw-semibold border" 
                            onClick={onClose} 
                            style={{ borderRadius: '6px', fontSize: '14px', color: '#333' }}
                        >
                            Cancel
                        </button>
                        ...
                        <button 
                            ...
                            onClick={handleActionPO} 
                            disabled={loading}
                        >
                            {loading 
                                ? (editingPO ? 'Updating...' : 'Saving...') 
